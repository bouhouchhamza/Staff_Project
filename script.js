...
function updateZonesAppearance() {
  document.querySelectorAll('.zone').forEach(zone => {
    ...
    const requiredZones = ['Réception', 'Salle des serveurs', 'Salle de sécurité', 'Salle d\'archives'];
    if (requiredZones.includes(zoneName) && currentWorkers === 0) {
...
add_worker_info.addEventListener("click", (e) => {
  ...
  const worker_img = document.getElementById("image-preview").value.trim() || "https://via.placeholder.com/60";
...
document.querySelectorAll(".assign-worker").forEach(button => {
  button.addEventListener("click", function() {
    ...
    const workerCard = document.querySelector(`[data-worker-id="${workerToAssign.id}"]`);
